...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    public void setState(TransitionState newState) {
        ...
        int i = C01935.f55xabc7e4ac[transitionState.ordinal()];
        if (i == 1 || i == 2) {
    ...
    /* loaded from: classes.dex */
    public static /* synthetic */ class C01935 {
    ...
    */
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if ((this.mDebugPath & 1) == 1 && !isInEditMode()) {
            ...
            sb.append(" ) state=");
            int i = this.mCurrentState;
            sb.append(i == -1 ? JsonSerializer.Undefined : Debug.getState(this, i));
            String sb2 = sb.toString();
    ...
    */
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attrs, C0209R.styleable.MotionLayout);
            int indexCount = obtainStyledAttributes.getIndexCount();
            boolean z = true;
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0209R.styleable.MotionLayout_layoutDescription) {
                    this.mScene = new MotionScene(getContext(), this, obtainStyledAttributes.getResourceId(index, -1));
                } else if (index == C0209R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = obtainStyledAttributes.getResourceId(index, -1);
                } else if (index == C0209R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = obtainStyledAttributes.getFloat(index, 0.0f);
                    this.mInTransition = true;
                } else if (index == C0209R.styleable.MotionLayout_applyMotionScene) {
                    z = obtainStyledAttributes.getBoolean(index, z);
                } else if (index == C0209R.styleable.MotionLayout_showPaths) {
                    if (this.mDebugPath == 0) {
                        this.mDebugPath = obtainStyledAttributes.getBoolean(index, false) ? 2 : 0;
                    }
                } else if (index == C0209R.styleable.MotionLayout_motionDebug) {
    ...
    public void updateStateAnimate(int stateId, ConstraintSet set, int duration) {
        if (this.mScene != null && this.mCurrentState == stateId) {
            updateState(C0209R.id.view_transition, getConstraintSet(stateId));
            setState(C0209R.id.view_transition, -1, -1);
            updateState(stateId, set);
            MotionScene.Transition transition = new MotionScene.Transition(-1, this.mScene, C0209R.id.view_transition, stateId);
