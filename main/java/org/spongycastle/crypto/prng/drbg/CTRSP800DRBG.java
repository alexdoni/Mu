...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    private void CTR_DRBG_Reseed_algorithm(byte[] bArr) {
        CTR_DRBG_Update(Block_Cipher_df(Arrays.concatenate(getEntropy(), bArr), this._seedLength), this._Key, this.f2528_V);
        this._reseedCounter = 1L;
    }
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (bArr2 != null) {
            ...
            CTR_DRBG_Update(bArr3, this._Key, this.f2528_V);
        } else {
            bArr3 = new byte[this._seedLength];
        }
        int length = this.f2528_V.length;
        byte[] bArr4 = new byte[length];
        this._engine.init(true, new KeyParameter(expandKey(this._Key)));
        for (int i = 0; i <= bArr.length / length; i++) {
            int i2 = i * length;
            int length2 = bArr.length - i2 > length ? length : bArr.length - (this.f2528_V.length * i);
            if (length2 != 0) {
                addOneTo(this.f2528_V);
                this._engine.processBlock(this.f2528_V, 0, bArr4, 0);
    ...
    private void padKey(byte[] bArr, int i, byte[] bArr2, int i2) {
        int i3 = i + 0;
        bArr2[i2 + 0] = (byte) (bArr[i3] & FileDownloadStatus.paused);
        int i4 = i + 1;
        bArr2[i2 + 1] = (byte) ((bArr[i3] << 7) | ((bArr[i4] & FileDownloadStatus.warn) >>> 1));
        int i5 = bArr[i4] << 6;
        int i6 = i + 2;
        ...
        while (i2 <= i14) {
            byte b = bArr2[i2];
            bArr2[i2] = (byte) (((((b >> 7) ^ ((((((b >> 1) ^ (b >> 2)) ^ (b >> 3)) ^ (b >> 4)) ^ (b >> 5)) ^ (b >> 6))) ^ 1) & 1) | (b & FileDownloadStatus.paused));
            i2++;
        }
    }
}
