...
public class ComputeInField {
    ...
    public short[] solveEquation(short[][] sArr, short[] sArr2) {
        ...
        try {
            this.f2847A = (short[][]) Array.newInstance((Class<?>) Short.TYPE, sArr.length, sArr.length + 1);
            this.f2848x = new short[sArr.length];
            for (int i = 0; i < sArr.length; i++) {
                for (int i2 = 0; i2 < sArr[0].length; i2++) {
                    this.f2847A[i][i2] = sArr[i][i2];
                }
            }
            for (int i3 = 0; i3 < sArr2.length; i3++) {
                short[] sArr3 = this.f2847A[i3];
    ...
    public short[][] inverse(short[][] sArr) {
        try {
            int i = 0;
            this.f2847A = (short[][]) Array.newInstance((Class<?>) Short.TYPE, sArr.length, sArr.length * 2);
            if (sArr.length != sArr[0].length) {
                throw new RuntimeException("The matrix is not invertible. Please choose another one!");
            }
            for (int i2 = 0; i2 < sArr.length; i2++) {
                for (int i3 = 0; i3 < sArr.length; i3++) {
                    this.f2847A[i2][i3] = sArr[i2][i3];
                }
                for (int length = sArr.length; length < sArr.length * 2; length++) {
                    this.f2847A[i2][length] = 0;
                }
                short[][] sArr2 = this.f2847A;
                sArr2[i2][sArr2.length + i2] = 1;
            }
            computeZerosUnder(true);
            int i4 = 0;
            while (true) {
                short[][] sArr3 = this.f2847A;
                if (i4 >= sArr3.length) {
                    break;
                }
    ...
    */
    ...
    private void computeZerosAbove() throws RuntimeException {
        for (int length = this.f2847A.length - 1; length > 0; length--) {
            for (int i = length - 1; i >= 0; i--) {
                short[][] sArr = this.f2847A;
    ...
    private void substitute() throws IllegalStateException {
        short invElem = GF2Field.invElem(this.f2847A[r0.length - 1][r0.length - 1]);
        if (invElem == 0) {
            throw new IllegalStateException("The equation system is not solvable");
        }
        short[] sArr = this.f2848x;
        short[][] sArr2 = this.f2847A;
        sArr[sArr2.length - 1] = GF2Field.multElem(sArr2[sArr2.length - 1][sArr2.length], invElem);
        for (int length = this.f2847A.length - 2; length >= 0; length--) {
            short[][] sArr3 = this.f2847A;
            short s = sArr3[length][sArr3.length];
            for (int length2 = sArr3.length - 1; length2 > length; length2--) {
                s = GF2Field.addElem(s, GF2Field.multElem(this.f2847A[length][length2], this.f2848x[length2]));
            }
            short invElem2 = GF2Field.invElem(this.f2847A[length][length]);
            if (invElem2 == 0) {
                throw new IllegalStateException("Not solvable equation system");
            }
            this.f2848x[length] = GF2Field.multElem(s, invElem2);
        }
    }
    ...
    public short[][] multiplyMatrix(short[][] sArr, short[][] sArr2) throws RuntimeException {
        ...
        this.f2847A = (short[][]) Array.newInstance((Class<?>) Short.TYPE, sArr.length, sArr2[0].length);
        for (int i = 0; i < sArr.length; i++) {
