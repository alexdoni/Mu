...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            ...
            while (it.hasNext()) {
                ...
                next.executor.execute(new CallResourceReady(next.f313cb));
            }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            while (it.hasNext()) {
                ResourceCallbackAndExecutor next = it.next();
                next.executor.execute(new CallLoadFailed(next.f313cb));
            }
            decrementPendingCallbacks();
        }
    }
    ...
    public class CallLoadFailed implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f311cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f311cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f311cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f311cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f311cb);
                    }
    ...
    public class CallResourceReady implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f312cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f312cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f312cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f312cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f312cb);
                        EngineJob.this.removeCallback(this.f312cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    public static final class ResourceCallbackAndExecutor {
        ...
        /* renamed from: cb */
        final ResourceCallback f313cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f313cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f313cb.equals(((ResourceCallbackAndExecutor) obj).f313cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f313cb.hashCode();
        }
    }
