...
public class FileDownloadSerialQueue {
    ...
    public void pause() {
        synchronized (this.finishCallback) {
            if (this.paused) {
                FileDownloadLog.m596w(this, "require pause this queue(remain %d), but it has already been paused", Integer.valueOf(this.mTasks.size()));
                return;
            }
            this.paused = true;
    ...
    public void resume() {
        synchronized (this.finishCallback) {
            if (!this.paused) {
                FileDownloadLog.m596w(this, "require resume this queue(remain %d), but it is still running", Integer.valueOf(this.mTasks.size()));
                return;
            }
            this.paused = false;
