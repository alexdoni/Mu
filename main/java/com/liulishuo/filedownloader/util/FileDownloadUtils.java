...
public class FileDownloadUtils {
    ...
    public static boolean isDownloaderProcess(Context context) {
        ...
        if (!FileDownloadProperties.getImpl().processNonSeparate) {
            ...
            if (activityManager == null) {
                FileDownloadLog.m596w(FileDownloadUtils.class, "fail to get the activity manager!", new Object[0]);
                return false;
            }
            List<ActivityManager.RunningAppProcessInfo> runningAppProcesses = activityManager.getRunningAppProcesses();
            if (runningAppProcesses == null || runningAppProcesses.isEmpty()) {
                FileDownloadLog.m596w(FileDownloadUtils.class, "The running app process info list from ActivityManager is null or empty, maybe current App is not running.", new Object[0]);
                return false;
            }
    ...
    public static long parseContentRangeFoInstanceLength(String str) {
        ...
        if (split.length >= 2) {
            try {
                ...
                FileDownloadLog.m596w(FileDownloadUtils.class, "parse instance length failed with %s", str);
            }
        }
        return -1L;
    }
    ...
    public static boolean isNetworkNotOnWifiType() {
        ...
        if (connectivityManager == null) {
            FileDownloadLog.m596w(FileDownloadUtils.class, "failed to get connectivity manager!", new Object[0]);
            return true;
        }
    ...
    public static String findEtag(int i, FileDownloadConnection fileDownloadConnection) {
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m591d(FileDownloadUtils.class, "etag find %s for task(%d)", responseHeaderField, Integer.valueOf(i));
        }
    ...
    public static long findInstanceLengthForTrial(FileDownloadConnection fileDownloadConnection) {
        ...
        if (findInstanceLengthFromContentRange < 0) {
            FileDownloadLog.m596w(FileDownloadUtils.class, "don't get instance length fromContent-Range header", new Object[0]);
    ...
    public static long findContentLength(int i, FileDownloadConnection fileDownloadConnection) {
        ...
        if (!(responseHeaderField != null && responseHeaderField.equals("chunked"))) {
            if (FileDownloadProperties.getImpl().httpLenient) {
                if (FileDownloadLog.NEED_LOG) {
                    FileDownloadLog.m591d(FileDownloadUtils.class, "%d response header is not legal but HTTP lenient is true, so handle as the case of transfer encoding chunk", Integer.valueOf(i));
                }
            } else {
    ...
    public static long parseContentLengthFromContentRange(String str) {
        if (str != null && str.length() != 0) {
            try {
                ...
                FileDownloadLog.m593e(FileDownloadUtils.class, e, "parse content length from content range error", new Object[0]);
            }
        }
        return -1L;
    }
    ...
    public static boolean isBreakpointAvailable(int i, FileDownloadModel fileDownloadModel, Boolean bool) {
        if (fileDownloadModel == null) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(FileDownloadUtils.class, "can't continue %d model == null", Integer.valueOf(i));
            }
            return false;
        }
        if (fileDownloadModel.getTempFilePath() == null) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(FileDownloadUtils.class, "can't continue %d temp path == null", Integer.valueOf(i));
            }
            return false;
        }
    ...
    public static boolean isBreakpointAvailable(int i, FileDownloadModel fileDownloadModel, String str, Boolean bool) {
        if (str == null) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(FileDownloadUtils.class, "can't continue %d path = null", Integer.valueOf(i));
            }
        } else {
            File file = new File(str);
            ...
            if (!exists || isDirectory) {
                if (FileDownloadLog.NEED_LOG) {
                    FileDownloadLog.m591d(FileDownloadUtils.class, "can't continue %d file not suit, exists[%B], directory[%B]", Integer.valueOf(i), Boolean.valueOf(exists), Boolean.valueOf(isDirectory));
                }
            } else {
                ...
                if (fileDownloadModel.getConnectionCount() <= 1 && soFar == 0) {
                    if (FileDownloadLog.NEED_LOG) {
                        FileDownloadLog.m591d(FileDownloadUtils.class, "can't continue %d the downloaded-record is zero.", Integer.valueOf(i));
                    }
                } else {
                    long total = fileDownloadModel.getTotal();
                    if (length < soFar || (total != -1 && (length > total || soFar >= total))) {
                        if (FileDownloadLog.NEED_LOG) {
                            FileDownloadLog.m591d(FileDownloadUtils.class, "can't continue %d dirty data fileLength[%d] sofar[%d] total[%d]", Integer.valueOf(i), Long.valueOf(length), Long.valueOf(soFar), Long.valueOf(total));
                        }
                    } else {
                        ...
                        if (FileDownloadLog.NEED_LOG) {
                            FileDownloadLog.m591d(FileDownloadUtils.class, "can't continue %d, because of the output stream doesn't support seek, but the task has already pre-allocated, so we only can download it from the very beginning.", Integer.valueOf(i));
                        }
                    }
                }
            }
