...
public class SqliteDatabaseImpl implements FileDownloadDatabase {
    ...
    /* renamed from: db */
    private final SQLiteDatabase f710db = new SqliteDatabaseOpenHelper(FileDownloadHelper.getAppContext()).getWritableDatabase();
    ...
    public FileDownloadModel find(int i) {
        ...
        Cursor cursor;
        try {
            cursor = this.f710db.rawQuery(FileDownloadUtils.formatString("SELECT * FROM %s WHERE %s = ?", TABLE_NAME, FileDownloadModel.f718ID), new String[]{Integer.toString(i)});
        } catch (Throwable th2) {
            th = th2;
            cursor = null;
        }
    ...
    public List<ConnectionModel> findConnectionModel(int i) {
        ...
        Cursor cursor = null;
        try {
            cursor = this.f710db.rawQuery(FileDownloadUtils.formatString("SELECT * FROM %s WHERE %s = ?", CONNECTION_TABLE_NAME, "id"), new String[]{Integer.toString(i)});
    ...
    public void removeConnections(int i) {
        this.f710db.execSQL("DELETE FROM filedownloaderConnection WHERE id = " + i);
    }

    @Override // com.liulishuo.filedownloader.database.FileDownloadDatabase
    public void insertConnectionModel(ConnectionModel connectionModel) {
        this.f710db.insert(CONNECTION_TABLE_NAME, null, connectionModel.toContentValues());
    }
    ...
    public void updateConnectionModel(int i, int i2, long j) {
        ...
        this.f710db.update(CONNECTION_TABLE_NAME, contentValues, "id = ? AND connectionIndex = ?", new String[]{Integer.toString(i), Integer.toString(i2)});
    }
    ...
    public void updateConnectionCount(int i, int i2) {
        ...
        this.f710db.update(TABLE_NAME, contentValues, "_id = ? ", new String[]{Integer.toString(i)});
    }

    @Override // com.liulishuo.filedownloader.database.FileDownloadDatabase
    public void insert(FileDownloadModel fileDownloadModel) {
        this.f710db.insert(TABLE_NAME, null, fileDownloadModel.toContentValues());
    }

    @Override // com.liulishuo.filedownloader.database.FileDownloadDatabase
    public void update(FileDownloadModel fileDownloadModel) {
        if (fileDownloadModel == null) {
            FileDownloadLog.m596w(this, "update but model == null!", new Object[0]);
        } else if (find(fileDownloadModel.getId()) != null) {
            this.f710db.update(TABLE_NAME, fileDownloadModel.toContentValues(), "_id = ? ", new String[]{String.valueOf(fileDownloadModel.getId())});
        } else {
    ...
    public boolean remove(int i) {
        return this.f710db.delete(TABLE_NAME, "_id = ?", new String[]{String.valueOf(i)}) != 0;
    }

    @Override // com.liulishuo.filedownloader.database.FileDownloadDatabase
    public void clear() {
        this.f710db.delete(TABLE_NAME, null, null);
        this.f710db.delete(CONNECTION_TABLE_NAME, null, null);
    }
    ...
    private void update(int i, ContentValues contentValues) {
        this.f710db.update(TABLE_NAME, contentValues, "_id = ? ", new String[]{String.valueOf(i)});
    }
    ...
    public class Maintainer implements FileDownloadDatabase.Maintainer {
        ...
        public void onFinishMaintain() {
            ...
            SqliteDatabaseImpl.this.f710db.beginTransaction();
            for (int i = 0; i < size; i++) {
                try {
                    int keyAt = this.needChangeIdList.keyAt(i);
                    FileDownloadModel fileDownloadModel = this.needChangeIdList.get(keyAt);
                    SqliteDatabaseImpl.this.f710db.delete(SqliteDatabaseImpl.TABLE_NAME, "_id = ?", new String[]{String.valueOf(keyAt)});
                    SqliteDatabaseImpl.this.f710db.insert(SqliteDatabaseImpl.TABLE_NAME, null, fileDownloadModel.toContentValues());
                    if (fileDownloadModel.getConnectionCount() > 1) {
                        ...
                        if (findConnectionModel.size() > 0) {
                            SqliteDatabaseImpl.this.f710db.delete(SqliteDatabaseImpl.CONNECTION_TABLE_NAME, "id = ?", new String[]{String.valueOf(keyAt)});
                            for (ConnectionModel connectionModel : findConnectionModel) {
                                connectionModel.setId(fileDownloadModel.getId());
                                SqliteDatabaseImpl.this.f710db.insert(SqliteDatabaseImpl.CONNECTION_TABLE_NAME, null, connectionModel.toContentValues());
                            }
                        }
                    }
                } finally {
                    SqliteDatabaseImpl.this.f710db.endTransaction();
                }
            }
            ...
            SqliteDatabaseImpl.this.f710db.setTransactionSuccessful();
        }
    ...
    class MaintainerIterator implements Iterator<FileDownloadModel> {
        ...
        /* renamed from: c */
        private final Cursor f711c;
        private int currentId;
        private final List<Integer> needRemoveId = new ArrayList();

        MaintainerIterator() {
            this.f711c = SqliteDatabaseImpl.this.f710db.rawQuery("SELECT * FROM filedownloader", null);
        }

        @Override // java.util.Iterator
        public boolean hasNext() {
            return this.f711c.moveToNext();
        }

        /* JADX WARN: Can't rename method to resolve collision */
        @Override // java.util.Iterator
        public FileDownloadModel next() {
            FileDownloadModel createFromCursor = SqliteDatabaseImpl.createFromCursor(this.f711c);
        ...
        void onFinishMaintain() {
            ...
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "delete %s", join);
            }
            SqliteDatabaseImpl.this.f710db.execSQL(FileDownloadUtils.formatString("DELETE FROM %s WHERE %s IN (%s);", SqliteDatabaseImpl.TABLE_NAME, FileDownloadModel.f718ID, join));
            SqliteDatabaseImpl.this.f710db.execSQL(FileDownloadUtils.formatString("DELETE FROM %s WHERE %s IN (%s);", SqliteDatabaseImpl.CONNECTION_TABLE_NAME, "id", join));
        }
    }
    ...
    public static FileDownloadModel createFromCursor(Cursor cursor) {
        ...
        fileDownloadModel.setId(cursor.getInt(cursor.getColumnIndex(FileDownloadModel.f718ID)));
