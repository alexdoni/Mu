...
public class DownloadTaskHunter implements ITaskHunter, ITaskHunter.IStarter, ITaskHunter.IMessageHandler, BaseDownloadTask.LifeCycleCallback {
    ...
    public boolean updateKeepAhead(MessageSnapshot messageSnapshot) {
        if (!FileDownloadStatus.isKeepAhead(getStatus(), messageSnapshot.getStatus())) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "can't update mStatus change by keep ahead, %d, but the current mStatus is %d, %d", Byte.valueOf(this.mStatus), Byte.valueOf(getStatus()), Integer.valueOf(getId()));
            }
            return false;
        }
    ...
    public boolean updateKeepFlow(MessageSnapshot messageSnapshot) {
        ...
        if (-2 == status && FileDownloadStatus.isIng(status2)) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "High concurrent cause, callback pending, but has already be paused %d", Integer.valueOf(getId()));
            }
            return true;
        }
        if (!FileDownloadStatus.isKeepFlow(status, status2)) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "can't update mStatus change by keep flow, %d, but the current mStatus is %d, %d", Byte.valueOf(this.mStatus), Byte.valueOf(getStatus()), Integer.valueOf(getId()));
            }
            return false;
        }
        update(messageSnapshot);
    ...
    private void update(MessageSnapshot messageSnapshot) {
        ...
        if (status == -4) {
            ...
            if (count + ((count > 1 || !origin.isPathAsDirectory()) ? 0 : FileDownloadList.getImpl().count(FileDownloadUtils.generateId(origin.getUrl(), origin.getTargetFilePath()))) <= 1) {
                ...
                FileDownloadLog.m596w(this, "warn, but no mListener to receive, switch to pending %d %d", Integer.valueOf(origin.getId()), Integer.valueOf(status2));
        ...
        if (status == 2) {
            ...
            if (fileName != null) {
                if (origin.getFilename() != null) {
                    FileDownloadLog.m596w(this, "already has mFilename[%s], but assign mFilename[%s] again", origin.getFilename(), fileName);
                }
    ...
    public void onBegin() {
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m595v(this, "filedownloader:lifecycle:start %s by %d ", toString(), Byte.valueOf(getStatus()));
        }
    }
    ...
    public void onOver() {
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m595v(this, "filedownloader:lifecycle:over %s by %d ", toString(), Byte.valueOf(getStatus()));
        }
    ...
    public void intoLaunchPool() {
        ...
        synchronized (this.mPauseLock) {
            if (this.mStatus != 0) {
                FileDownloadLog.m596w(this, "High concurrent cause, this task %d will not input to launch pool, because of the status isn't idle : %d", Integer.valueOf(getId()), Byte.valueOf(this.mStatus));
                return;
            }
            ...
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m595v(this, "call start Url[%s], Path[%s] Listener[%s], Tag[%s]", origin.getUrl(), origin.getPath(), origin.getListener(), origin.getTag());
            }
            try {
                prepare();
                z = true;
            ...
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m595v(this, "the task[%d] has been into the launch pool.", Integer.valueOf(getId()));
            }
        }
    }
    ...
    public boolean pause() {
        if (FileDownloadStatus.isOver(getStatus())) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "High concurrent cause, Already is over, can't pause again, %d %d", Byte.valueOf(getStatus()), Integer.valueOf(this.mTask.getRunningTask().getOrigin().getId()));
            }
            return false;
        }
        this.mStatus = (byte) -2;
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m595v(this, "the task[%d] has been expired from the launch pool.", Integer.valueOf(getId()));
        }
        if (!FileDownloader.getImpl().isServiceConnected()) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "request pause the task[%d] to the download service, but the download service isn't connected yet.", Integer.valueOf(origin.getId()));
            }
        } else {
    ...
    public void free() {
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m591d(this, "free the task %d, when the status is %d", Integer.valueOf(getId()), Byte.valueOf(this.mStatus));
        }
        this.mStatus = (byte) 0;
    }
    ...
    private void prepare() throws IOException {
        ...
        if (origin.getPath() == null) {
            ...
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "save Path is null to %s", origin.getPath());
            }
        }
    ...
    public void start() {
        if (this.mStatus != 10) {
            FileDownloadLog.m596w(this, "High concurrent cause, this task %d will not start, because the of status isn't toLaunchPool: %d", Integer.valueOf(getId()), Byte.valueOf(this.mStatus));
            return;
        }
        BaseDownloadTask.IRunningTask runningTask = this.mTask.getRunningTask();
        BaseDownloadTask origin = runningTask.getOrigin();
        ...
        try {
            ...
            synchronized (this.mPauseLock) {
                if (this.mStatus != 10) {
                    FileDownloadLog.m596w(this, "High concurrent cause, this task %d will not start, the status can't assign to toFileDownloadService, because the status isn't toLaunchPool: %d", Integer.valueOf(getId()), Byte.valueOf(this.mStatus));
                    return;
                }
                ...
                if (this.mStatus == -2) {
                    FileDownloadLog.m596w(this, "High concurrent cause, this task %d will be paused,because of the status is paused, so the pause action must be applied", Integer.valueOf(getId()));
                    if (start) {
