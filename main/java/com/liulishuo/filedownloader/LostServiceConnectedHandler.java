...
public class LostServiceConnectedHandler extends FileDownloadConnectListener implements ILostServiceConnectedHandler {
    ...
    public void connected() {
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m591d(this, "The downloader service is connected.", new Object[0]);
        }
    ...
    public void disconnected() {
        if (getConnectStatus() == DownloadServiceConnectChangedEvent.ConnectStatus.lost) {
            IQueuesHandler queuesHandler = FileDownloader.getImpl().getQueuesHandler();
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "lost the connection to the file download service, and current active task size is %d", Integer.valueOf(FileDownloadList.getImpl().size()));
            }
            if (FileDownloadList.getImpl().size() > 0) {
                ...
                try {
                    ...
                    FileDownloadLog.m596w(this, "restart service failed, you may need to restart downloading manually when the app comes back to foreground", new Object[0]);
                    return;
                }
            }
            return;
        ...
        if (FileDownloadList.getImpl().size() > 0) {
            FileDownloadLog.m596w(this, "file download service has be unbound but the size of active tasks are not empty %d ", Integer.valueOf(FileDownloadList.getImpl().size()));
        }
    }
    ...
    public boolean dispatchTaskStart(BaseDownloadTask.IRunningTask iRunningTask) {
        if (!FileDownloader.getImpl().isServiceConnected()) {
            synchronized (this.mWaitingList) {
                if (!FileDownloader.getImpl().isServiceConnected()) {
                    if (FileDownloadLog.NEED_LOG) {
                        FileDownloadLog.m591d(this, "Waiting for connecting with the downloader service... %d", Integer.valueOf(iRunningTask.getOrigin().getId()));
                    }
