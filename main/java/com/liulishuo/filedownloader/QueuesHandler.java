...
public class QueuesHandler implements IQueuesHandler {
    ...
    private boolean onAssembledTasksToStart(int i, List<BaseDownloadTask.IRunningTask> list, FileDownloadListener fileDownloadListener, boolean z) {
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m595v(FileDownloader.class, "start list attachKey[%d] size[%d] listener[%s] isSerial[%B]", Integer.valueOf(i), Integer.valueOf(list.size()), fileDownloadListener, Boolean.valueOf(z));
        }
        if (list != null && !list.isEmpty()) {
            return false;
        }
        FileDownloadLog.m596w(FileDownloader.class, "Tasks with the listener can't start, because can't find any task with the provided listener, maybe tasks instance has been started in the past, so they are all are inUsing, if in this case, you can use [BaseDownloadTask#reuse] to reuse theme first then start again: [%s, %B]", fileDownloadListener, Boolean.valueOf(z));
        return true;
    }
    ...
    private class SerialHandlerCallback implements Handler.Callback {
            ...
            if (message.what == 1) {
                if (message.arg1 >= this.mList.size()) {
                    ...
                    if (FileDownloadLog.NEED_LOG) {
                        ...
                        FileDownloadLog.m591d(SerialHandlerCallback.class, "final serial %s %d", objArr);
                    }
                    return true;
                }
                int i = message.arg1;
                ...
                synchronized (iRunningTask.getPauseLock()) {
                    ...
                    if (FileDownloadLog.NEED_LOG) {
                        FileDownloadLog.m591d(SerialHandlerCallback.class, "direct go next by not contains %s %d", iRunningTask, Integer.valueOf(message.arg1));
                    }
        ...
        public void goNext(int i) {
            ...
            if (handler == null || this.mList == null) {
                FileDownloadLog.m596w(this, "need go next %d, but params is not ready %s %s", Integer.valueOf(i), this.mHandler, this.mList);
                return;
            }
            ...
            if (FileDownloadLog.NEED_LOG) {
                ...
                FileDownloadLog.m591d(SerialHandlerCallback.class, "start next %s %s", objArr);
            }
