...
public class FileDownloadList {
    ...
    public List<BaseDownloadTask.IRunningTask> getReceiveServiceTaskList(int i) {
        ...
        synchronized (this.mList) {
            ...
            while (it.hasNext()) {
                BaseDownloadTask.IRunningTask next = it.next();
                if (next.mo589is(i) && !next.isOver() && (status = next.getOrigin().getStatus()) != 0 && status != 10) {
                    arrayList.add(next);
                }
            }
        }
    ...
    public List<BaseDownloadTask.IRunningTask> copy(FileDownloadListener fileDownloadListener) {
        ...
        synchronized (this.mList) {
            ...
            while (it.hasNext()) {
                BaseDownloadTask.IRunningTask next = it.next();
                if (next.mo590is(fileDownloadListener)) {
                    arrayList.add(next);
                }
            }
        }
    ...
    public boolean remove(BaseDownloadTask.IRunningTask iRunningTask, MessageSnapshot messageSnapshot) {
        ...
        if (FileDownloadLog.NEED_LOG && this.mList.size() == 0) {
            FileDownloadLog.m595v(this, "remove %s left %d %d", iRunningTask, Byte.valueOf(status), Integer.valueOf(this.mList.size()));
        }
        if (remove) {
            ...
            FileDownloadLog.m592e(this, "remove error, not exist: %s %d", iRunningTask, Byte.valueOf(status));
        }
        return remove;
    }
    ...
    public void addUnchecked(BaseDownloadTask.IRunningTask iRunningTask) {
        ...
        synchronized (this.mList) {
            if (this.mList.contains(iRunningTask)) {
                FileDownloadLog.m596w(this, "already has %s", iRunningTask);
            } else {
                iRunningTask.markAdded2List();
                this.mList.add(iRunningTask);
                if (FileDownloadLog.NEED_LOG) {
                    FileDownloadLog.m595v(this, "add list in all %s %d %d", iRunningTask, Byte.valueOf(iRunningTask.getOrigin().getStatus()), Integer.valueOf(this.mList.size()));
                }
            }
        }
    }
