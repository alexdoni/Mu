            ...
            try {
                try {
                    ...
                    if (FileDownloadLog.NEED_LOG) {
                        FileDownloadLog.m591d(this, "the connection[%d] for %d, is connected %s with code[%d]", Integer.valueOf(this.connectionIndex), Integer.valueOf(this.downloadId), this.connectTask.getProfile(), Integer.valueOf(responseCode));
                    }
                    ...
                    try {
                        ...
                        try {
                            if (!this.callback.isRetry(e)) {
                                ...
                                FileDownloadLog.m596w(this, "it is valid to retry and connection is valid but create fetch-data-task failed, so give up directly with %s", e);
                                this.callback.onError(e);
                                if (fileDownloadConnection == null) {
                                    return;
                                }
