...
public class ConnectTask {
    ...
    public void updateConnectionProfile(long j) {
        if (j == this.profile.currentOffset) {
            FileDownloadLog.m596w(this, "no data download, no need to update", new Object[0]);
            return;
        }
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m594i(this, "after update profile:%s", this.profile);
        }
    }
    ...
    public FileDownloadConnection connect() throws IOException, IllegalAccessException {
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m591d(this, "<---- %s request header %s", Integer.valueOf(this.downloadId), this.requestHeader);
        }
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m591d(this, "----> %s response header %s", Integer.valueOf(this.downloadId), process.getResponseHeaderFields());
        }
        return process;
    }
    ...
    private void addUserRequiredHeader(FileDownloadConnection fileDownloadConnection) {
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m595v(this, "%d add outside header: %s", Integer.valueOf(this.downloadId), headers);
        }
