    ...
    */
    ...
    private void fetchWithMultipleConnection(List<ConnectionModel> list, long j) throws InterruptedException {
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m591d(this, "fetch data with multiple connection(count: [%d]) for task[%d] totalLength[%d]", Integer.valueOf(list.size()), Integer.valueOf(id), Long.valueOf(j));
        }
        ...
        for (ConnectionModel connectionModel : list) {
            ...
            if (j4 == j2) {
                if (FileDownloadLog.NEED_LOG) {
                    FileDownloadLog.m591d(this, "pass connection[%d-%d], because it has been completed", Integer.valueOf(connectionModel.getId()), Integer.valueOf(connectionModel.getIndex()));
                }
            } else {
                ...
                if (FileDownloadLog.NEED_LOG) {
                    FileDownloadLog.m591d(this, "enable multiple connection: %s", connectionModel);
                }
                if (build == null) {
        ...
        if (j3 != this.model.getSoFar()) {
            FileDownloadLog.m596w(this, "correct the sofar[%d] from connection table[%d]", Long.valueOf(this.model.getSoFar()), Long.valueOf(j3));
        ...
        if (FileDownloadLog.NEED_LOG) {
            for (Future future : invokeAll) {
                FileDownloadLog.m591d(this, "finish sub-task for [%d] %B %B", Integer.valueOf(id), Boolean.valueOf(future.isDone()), Boolean.valueOf(future.isCancelled()));
            }
        }
    }
    ...
    public void onCompleted(DownloadRunnable downloadRunnable, long j, long j2) {
        if (this.paused) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "the task[%d] has already been paused, so pass the completed callback", Integer.valueOf(this.model.getId()));
                return;
            }
            return;
        }
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m591d(this, "the connection has been completed(%d): [%d, %d)  %d", Integer.valueOf(i), Long.valueOf(j), Long.valueOf(j2), Long.valueOf(this.model.getTotal()));
        }
        if (!this.isSingleConnection) {
            ...
            FileDownloadLog.m592e(this, "the single task not completed corrected(%d, %d != %d) for task(%d)", Long.valueOf(j), Long.valueOf(j2), Long.valueOf(this.model.getTotal()), Integer.valueOf(this.model.getId()));
        }
    }
    ...
    public void onError(Exception exc) {
        ...
        if (this.paused) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "the task[%d] has already been paused, so pass the error callback", Integer.valueOf(this.model.getId()));
            }
        } else {
            Iterator it = ((ArrayList) this.downloadRunnableList.clone()).iterator();
            while (it.hasNext()) {
    ...
    public void onRetry(Exception exc) {
        if (this.paused) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "the task[%d] has already been paused, so pass the retry callback", Integer.valueOf(this.model.getId()));
            }
        } else {
            ...
            if (i < 0) {
                FileDownloadLog.m592e(this, "valid retry times is less than 0(%d) for download task(%d)", Integer.valueOf(i2), Integer.valueOf(this.model.getId()));
            }
