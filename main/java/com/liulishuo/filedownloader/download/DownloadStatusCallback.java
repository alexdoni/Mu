...
public class DownloadStatusCallback implements Handler.Callback {
    ...
    private synchronized void sendMessage(Message message) {
        if (!this.handlerThread.isAlive()) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, ALREADY_DEAD_MESSAGE, Integer.valueOf(message.what));
            }
            return;
        }
        try {
    ...
    private Exception exFiltrate(Exception exc) {
        ...
        if (!file.exists()) {
            FileDownloadLog.m593e(this, exc, "Exception with: free space isn't enough, and the target file not exist.", new Object[0]);
            length = 0;
        } else {
            length = file.length();
        }
    ...
    private void handleSQLiteFullException(SQLiteFullException sQLiteFullException) {
        ...
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m591d(this, "the data of the task[%d] is dirty, because the SQLite full exception[%s], so remove it from the database directly.", Integer.valueOf(id), sQLiteFullException.toString());
        }
    ...
    private void renameTempFile() throws IOException {
        ...
        try {
            ...
            if (file2.exists()) {
                ...
                FileDownloadLog.m596w(this, "The target file([%s], [%d]) will be replaced with the new downloaded file[%d]", targetFilePath, Long.valueOf(length), Long.valueOf(file.length()));
            }
            ...
            if (z) {
                try {
                    ...
                    if (z && file.exists() && !file.delete()) {
                        FileDownloadLog.m596w(this, "delete the temp file(%s) failed, on completed downloading.", tempFilePath);
                    }
                    throw th;
                }
            }
    ...
    */
    ...
    private void handleProgress() {
        ...
        if (this.needSetProcess.compareAndSet(true, false)) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m594i(this, "handleProgress update model's status with progress", new Object[0]);
            }
            this.model.setStatus((byte) 3);
        }
        if (this.needCallbackProgressToUser.compareAndSet(true, false)) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m594i(this, "handleProgress notify user progress status", new Object[0]);
            }
    ...
    private void inspectNeedCallbackToUser(long j) {
        ...
        if (!this.isFirstCallback.compareAndSet(true, false)) {
            ...
            if (this.callbackMinIntervalBytes == -1 || this.callbackIncreaseBuffer.get() < this.callbackMinIntervalBytes || j2 < this.callbackProgressMinInterval) {
                ...
                if (FileDownloadLog.NEED_LOG) {
                    FileDownloadLog.m594i(this, "inspectNeedCallbackToUser need callback to user", new Object[0]);
                }
    ...
    private void onStatusChanged(byte b) {
        if (b == -2) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.m591d(this, "High concurrent cause, Already paused and we don't need to call-back to Task in here, %d", Integer.valueOf(this.model.getId()));
                return;
            }
            return;
        }
