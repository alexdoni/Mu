...
public class FileDownloader {
    ...
    public static void init(Context context, DownloadMgrInitialParams.InitCustomMaker initCustomMaker) {
        if (FileDownloadLog.NEED_LOG) {
            FileDownloadLog.m591d(FileDownloader.class, "init Downloader with params: %s %s", context, initCustomMaker);
        }
        if (context == null) {
            throw new IllegalArgumentException("the provided context must not be null!");
        }
    ...
    public boolean start(FileDownloadListener fileDownloadListener, boolean z) {
        if (fileDownloadListener == null) {
            FileDownloadLog.m596w(this, "Tasks with the listener can't start, because the listener provided is null: [null, %B]", Boolean.valueOf(z));
            return false;
        }
        if (z) {
    ...
    public int pause(int i) {
        ...
        if (downloadingList == null || downloadingList.isEmpty()) {
            FileDownloadLog.m596w(this, "request pause but not exist %d", Integer.valueOf(i));
            return 0;
        }
    ...
    public void addServiceConnectListener(FileDownloadConnectListener fileDownloadConnectListener) {
        FileDownloadEventPool.getImpl().addListener(DownloadServiceConnectChangedEvent.f713ID, fileDownloadConnectListener);
    }

    public void removeServiceConnectListener(FileDownloadConnectListener fileDownloadConnectListener) {
        FileDownloadEventPool.getImpl().removeListener(DownloadServiceConnectChangedEvent.f713ID, fileDownloadConnectListener);
    }
    ...
    public boolean setTaskCompleted(String str, String str2, long j) {
        FileDownloadLog.m596w(this, "If you invoked this method, please remove it directly feel free, it doesn't need any longer", new Object[0]);
        return true;
    }
    ...
    public boolean setMaxNetworkThreadCount(int i) {
        if (!FileDownloadList.getImpl().isEmpty()) {
            FileDownloadLog.m596w(this, "Can't change the max network thread count, because there are actively executing tasks in FileDownloader, please try again after all actively executing tasks are completed or invoking FileDownloader#pauseAll directly.", new Object[0]);
            return false;
        }
