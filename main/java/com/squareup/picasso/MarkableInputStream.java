...
final class MarkableInputStream extends InputStream {
    ...
    /* renamed from: in */
    private final InputStream f812in;
    private long limit;
    ...
    private MarkableInputStream(InputStream inputStream, int i, int i2) {
        ...
        this.f812in = inputStream.markSupported() ? inputStream : new BufferedInputStream(inputStream, i);
    ...
    private void setLimit(long j) {
        try {
            ...
            if (j2 < j3 && j3 <= this.limit) {
                this.f812in.reset();
                this.f812in.mark((int) (j - this.reset));
                skip(this.reset, this.offset);
            } else {
                this.reset = j3;
                this.f812in.mark((int) (j - j3));
            }
            this.limit = j;
        } catch (IOException e) {
    ...
    private void skip(long j, long j2) throws IOException {
        while (j < j2) {
            long skip = this.f812in.skip(j2 - j);
            if (skip == 0) {
                if (read() == -1) {
                    return;
                } else {
    ...
    public int read(byte[] bArr) throws IOException {
        ...
        int read = this.f812in.read(bArr);
        if (read != -1) {
            this.offset += read;
        }
        return read;
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        ...
        int read = this.f812in.read(bArr, i, i2);
        if (read != -1) {
            this.offset += read;
        }
        return read;
    ...
    public boolean markSupported() {
        return this.f812in.markSupported();
    }
}
