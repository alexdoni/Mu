...
import com.p017xx.commom.glide.load.MultiTransformation;
...
import com.p017xx.commom.glide.load.resource.bitmap.CenterCrop;
import com.p017xx.commom.glide.load.resource.bitmap.CenterInside;
import com.p017xx.commom.glide.load.resource.bitmap.CircleCrop;
...
import com.p017xx.commom.glide.load.resource.bitmap.DrawableTransformation;
import com.p017xx.commom.glide.load.resource.bitmap.FitCenter;
...
import com.p017xx.commom.glide.load.resource.gif.GifDrawableTransformation;
import com.p017xx.commom.glide.load.resource.gif.GifOptions;
import com.p017xx.commom.glide.request.BaseRequestOptions;
import com.p017xx.commom.glide.signature.EmptySignature;
import com.p017xx.commom.glide.util.CachedHashCodeArrayMap;
...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
    ...
    public T sizeMultiplier(float f) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().sizeMultiplier(f);
        }
        if (f < 0.0f || f > 1.0f) {
    ...
    public T useUnlimitedSourceGeneratorsPool(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().useUnlimitedSourceGeneratorsPool(z);
        }
    ...
    public T useAnimationPool(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().useAnimationPool(z);
        }
    ...
    public T onlyRetrieveFromCache(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().onlyRetrieveFromCache(z);
        }
    ...
    public T diskCacheStrategy(DiskCacheStrategy diskCacheStrategy) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().diskCacheStrategy(diskCacheStrategy);
        }
    ...
    public T priority(Priority priority) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().priority(priority);
        }
    ...
    public T placeholder(Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().placeholder(drawable);
        }
    ...
    public T placeholder(int i) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().placeholder(i);
        }
    ...
    public T fallback(Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().fallback(drawable);
        }
    ...
    public T fallback(int i) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().fallback(i);
        }
        this.fallbackId = i;
    ...
    public T error(Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().error(drawable);
        }
    ...
    public T theme(Resources.Theme theme) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().theme(theme);
        }
        this.theme = theme;
        this.fields |= 32768;
        return selfOrThrowIfLocked();
    ...
    public T skipMemoryCache(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().skipMemoryCache(true);
        }
        this.isCacheable = !z;
        this.fields |= 256;
        return selfOrThrowIfLocked();
    ...
    public T override(int i, int i2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().override(i, i2);
        }
    ...
    public T signature(Key key) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().signature(key);
        }
    ...
    public <Y> T set(Option<Y> option, Y y) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().set(option, y);
        }
    ...
    public T decode(Class<?> cls) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().decode(cls);
        }
    ...
    final T optionalTransform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().optionalTransform(downsampleStrategy, transformation);
        }
    ...
    final T transform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().transform(downsampleStrategy, transformation);
        }
        downsample(downsampleStrategy);
    ...
    T transform(Transformation<Bitmap> transformation, boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().transform(transformation, z);
        }
    ...
    <Y> T transform(Class<Y> cls, Transformation<Y> transformation, boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().transform(cls, transformation, z);
        }
    ...
    public T dontTransform() {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().dontTransform();
        }
    ...
    public T apply(BaseRequestOptions<?> baseRequestOptions) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1876clone().apply(baseRequestOptions);
        }
