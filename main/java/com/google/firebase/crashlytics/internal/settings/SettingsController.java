...
public class SettingsController implements SettingsProvider {
                ...
                if (readCachedSettings != null) {
                    ...
                    if (parseSettingsJson != null) {
                        ...
                        if (!SettingsCacheBehavior.IGNORE_CACHE_EXPIRATION.equals(settingsCacheBehavior) && parseSettingsJson.isExpired(currentTimeMillis)) {
                            Logger.getLogger().m544v("Cached settings have expired.");
                        }
                        try {
                            Logger.getLogger().m544v("Returning cached settings.");
                            settings = parseSettingsJson;
                        } catch (Exception e) {
                            e = e;
                            settings = parseSettingsJson;
                            Logger.getLogger().m541e("Failed to get cached settings", e);
                            return settings;
                        }
                    } else {
                        Logger.getLogger().m541e("Failed to parse cached settings data.", null);
                    }
                } else {
                    Logger.getLogger().m538d("No cached settings data found.");
                }
            }
        } catch (Exception e2) {
            e = e2;
    ...
    public void logSettings(JSONObject jSONObject, String str) throws JSONException {
        Logger.getLogger().m538d(str + jSONObject.toString());
    }
