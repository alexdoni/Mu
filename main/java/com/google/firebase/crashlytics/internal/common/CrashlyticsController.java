...
public class CrashlyticsController {
    ...
    synchronized void handleUncaughtException(final SettingsProvider settingsProvider, final Thread thread, final Throwable th, final boolean z) {
        Logger.getLogger().m538d("Handling uncaught exception \"" + th + "\" from thread " + thread.getName());
            ...
            try {
                Utils.awaitEvenIfOnMainThread(this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2
                    ...
                    public Task<Void> call() throws Exception {
                            ...
                            if (CrashlyticsController.this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
                                ...
                                return settingsProvider.getSettingsAsync().onSuccessTask(executor, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2.1
                                    ...
                                    public Task<Void> then(Settings settings) throws Exception {
                                        ...
                                        Logger.getLogger().m546w("Received null app settings, cannot send reports at crash time.");
                        ...
                        Logger.getLogger().m540e("Tried to write a fatal exception while no session was open.");
                        return Tasks.forResult(null);
                    }
                }));
            } catch (Exception e) {
                Logger.getLogger().m541e("Error handling uncaught exception", e);
            }
        } catch (TimeoutException unused) {
            Logger.getLogger().m540e("Cannot send reports. Timed out while fetching settings.");
        }
    }

    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().m538d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(true);
        }
        Logger.getLogger().m538d("Automatic data collection is disabled.");
        Logger.getLogger().m544v("Notifying that unsent reports are available.");
        ...
        Logger.getLogger().m538d("Waiting for send/deleteUnsentReports to be called.");
    ...
    public boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().m544v("Found previous crash marker.");
    ...
    public Task<Boolean> checkForUnsentReports() {
        if (!this.checkForUnsentReportsCalled.compareAndSet(false, true)) {
            Logger.getLogger().m546w("checkForUnsentReports should only be called once per execution.");
    ...
    public Task<Void> submitAllReports(Task<Settings> task) {
        if (!this.reportingCoordinator.hasReportsToSend()) {
            Logger.getLogger().m544v("No crash reports are available to be sent.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(null);
        }
        Logger.getLogger().m544v("Crash reports are available to be sent.");
        return waitForReportAction().onSuccessTask(new C15944(task));
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    /* renamed from: com.google.firebase.crashlytics.internal.common.CrashlyticsController$4 */
    /* loaded from: classes2.dex */
    public class C15944 implements SuccessContinuation<Boolean, Void> {
            ...
            return CrashlyticsController.this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1
                ...
                public Task<Void> call() throws Exception {
                    if (!bool.booleanValue()) {
                        Logger.getLogger().m544v("Deleting cached crash reports...");
                        CrashlyticsController.deleteFiles(CrashlyticsController.this.listAppExceptionMarkerFiles());
                        CrashlyticsController.this.reportingCoordinator.removeAllReports();
                        CrashlyticsController.this.unsentReportsHandled.trySetResult(null);
                        return Tasks.forResult(null);
                    }
                    Logger.getLogger().m538d("Sending cached crash reports...");
                    CrashlyticsController.this.dataCollectionArbiter.grantDataCollectionPermission(bool.booleanValue());
                    final Executor executor = CrashlyticsController.this.backgroundWorker.getExecutor();
                    return C15944.this.val$settingsDataTask.onSuccessTask(executor, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1.1
                        @Override // com.google.android.gms.tasks.SuccessContinuation
                        public Task<Void> then(Settings settings) throws Exception {
                            if (settings != null) {
                            ...
                            Logger.getLogger().m546w("Received null app settings at app startup. Cannot send cached reports");
                            return Tasks.forResult(null);
                        }
                    });
                }
        ...
        this.backgroundWorker.submit(new Runnable() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.6
            ...
            public void run() {
                ...
                if (currentSessionId != null) {
                    ...
                    Logger.getLogger().m546w("Tried to write a non-fatal exception while no session was open.");
                }
            }
        });
    }
    ...
    public void logFatalException(Thread thread, Throwable th) {
        ...
        if (settingsProvider == null) {
            Logger.getLogger().m546w("settingsProvider not set");
        } else {
    ...
    public void setCustomKey(String str, String str2) {
        try {
            ...
            Logger.getLogger().m540e("Attempting to set custom attribute with null key, ignoring.");
        }
    }
    ...
    public boolean finalizeSessions(SettingsProvider settingsProvider) {
        ...
        if (isHandlingException()) {
            Logger.getLogger().m546w("Skipping session finalization because a crash has already occurred.");
            Boolean bool = Boolean.FALSE;
            return false;
        }
        Logger.getLogger().m544v("Finalizing previously open sessions.");
        try {
            doCloseSessions(true, settingsProvider);
            Logger.getLogger().m544v("Closed all previously open sessions.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m541e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void doOpenSession(String str, Boolean bool) {
        ...
        Logger.getLogger().m538d("Opening a new session with ID " + str);
    ...
    private void doCloseSessions(boolean z, SettingsProvider settingsProvider) {
        ...
        if (arrayList.size() <= z) {
            Logger.getLogger().m544v("No open sessions to be closed.");
            return;
        }
        ...
        if (settingsProvider.getSettingsSync().featureFlagData.collectAnrs) {
            ...
            Logger.getLogger().m544v("ANR feature disabled.");
        }
    ...
    public void saveVersionControlInfo() {
        try {
            ...
            if (versionControlInfo != null) {
                ...
                Logger.getLogger().m542i("Saved version control info");
            }
        } catch (IOException e) {
            Logger.getLogger().m547w("Unable to save version control info", e);
        }
    }
    ...
    String getVersionControlInfo() throws IOException {
        ...
        Logger.getLogger().m538d("Read version control info");
    ...
    private InputStream getResourceAsStream(String str) {
        ...
        if (classLoader == null) {
            Logger.getLogger().m546w("Couldn't get Class Loader");
            return null;
        }
        ...
        Logger.getLogger().m542i("No version control information found");
        return null;
    }
    ...
    private void finalizePreviousNativeSession(String str) {
        Logger.getLogger().m544v("Finalizing native report for session " + str);
        ...
        if (nativeCoreAbsent(str, minidumpFile, applicationExitInto)) {
            Logger.getLogger().m546w("No native core present");
            return;
        }
        ...
        if (!nativeSessionDir.isDirectory()) {
            Logger.getLogger().m546w("Couldn't create directory to store native session files, aborting.");
            return;
        }
        ...
        Logger.getLogger().m538d("CrashlyticsController#finalizePreviousNativeSession");
    ...
    private static boolean nativeCoreAbsent(String str, File file, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        if (file == null || !file.exists()) {
            Logger.getLogger().m546w("No minidump data found for session " + str);
        }
        if (applicationExitInfo == null) {
            Logger.getLogger().m542i("No Tombstones data found for session " + str);
        }
    ...
    public void doWriteAppExceptionMarker(long j) {
        try {
            ...
            Logger.getLogger().m547w("Could not create app exception marker file.", e);
        }
    }
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File file : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger.getLogger().m546w("Could not parse app exception timestamp from file " + file.getName());
            }
            file.delete();
        }
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        if (firebaseCrashExists()) {
            Logger.getLogger().m546w("Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
            return Tasks.forResult(null);
        }
        Logger.getLogger().m538d("Logging app exception event to Firebase Analytics");
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        if (Build.VERSION.SDK_INT >= 30) {
            ...
            if (historicalProcessExitReasons.size() != 0) {
                ...
                Logger.getLogger().m544v("No ApplicationExitInfo available. Session: " + str);
                return;
            }
        }
        Logger.getLogger().m544v("ANR feature enabled, but device is API " + Build.VERSION.SDK_INT);
    }
}
