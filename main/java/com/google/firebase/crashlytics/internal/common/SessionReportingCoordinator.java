...
public class SessionReportingCoordinator implements CrashlyticsLifecycleEvents {
    ...
    public void persistFatalEvent(Throwable th, Thread thread, String str, long j) {
        Logger.getLogger().m544v("Persisting fatal event for session " + str);
        persistEvent(th, thread, str, "crash", j, true);
    }

    public void persistNonFatalEvent(Throwable th, Thread thread, String str, long j) {
        Logger.getLogger().m544v("Persisting non-fatal event for session " + str);
    ...
    public void persistRelevantAppExitInfoEvent(String str, List<ApplicationExitInfo> list, LogFileManager logFileManager, UserMetadata userMetadata) {
        ...
        if (findRelevantApplicationExitInfo == null) {
            Logger.getLogger().m544v("No relevant ApplicationExitInfo occurred during session: " + str);
            return;
        }
        ...
        Logger.getLogger().m538d("Persisting anr for session " + str);
    ...
    public void finalizeSessionWithNativeEvent(String str, List<NativeSessionFile> list, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        Logger.getLogger().m538d("SessionReportingCoordinator#finalizeSessionWithNativeEvent");
    ...
    private CrashlyticsReport.Session.Event addLogsAndCustomKeysToEvent(CrashlyticsReport.Session.Event event, LogFileManager logFileManager, UserMetadata userMetadata) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().m544v("No log data to include with this event.");
        }
    ...
    public boolean onReportSendComplete(Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            Logger.getLogger().m538d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
            File reportFile = result.getReportFile();
            if (reportFile.delete()) {
                Logger.getLogger().m538d("Deleted report file: " + reportFile.getPath());
                return true;
            }
            Logger.getLogger().m546w("Crashlytics could not delete report file: " + reportFile.getPath());
            return true;
        }
        Logger.getLogger().m547w("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }
    ...
    private static CrashlyticsReport.ApplicationExitInfo convertApplicationExitInfo(ApplicationExitInfo applicationExitInfo) {
        ...
        try {
            ...
            Logger.getLogger().m546w("Could not get input trace in application exit info: " + applicationExitInfo.toString() + " Error: " + e);
        }
