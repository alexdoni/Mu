...
public final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N n, N n2, E e) {
        ...
        if (containsEdge(e)) {
            ...
            EndpointPair m477of = EndpointPair.m477of(this, n, n2);
            Preconditions.checkArgument(incidentNodes.equals(m477of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", e, incidentNodes, m477of);
            return false;
        }
    ...
    private NetworkConnections<N, E> newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m474of();
            }
            return DirectedNetworkConnections.m475of();
        }
        if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m480of();
        }
        return UndirectedNetworkConnections.m481of();
    }
}
